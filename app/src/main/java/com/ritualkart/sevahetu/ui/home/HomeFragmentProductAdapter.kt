...
import com.ritualkart.sevahetu.databinding.ItemHomeProductBinding
...
class HomeFragmentProductAdapter(
    private val context: Context,
    private var orderList: List<ProductItemResponse>,
    private var sharedPreference: Preference,
    var categoryClickListener: HomeFragmentProductInterface
) : RecyclerView.Adapter<HomeFragmentProductAdapter.ProductItemViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ProductItemViewHolder, position: Int) {
        with(holder) {
            ...
            binding.tvActualPrice.text = "₹ " + orderList[position].price.toString() ?: ""
            binding.tvMarketPrice.text = "₹ " + orderList[position].price.toString() ?: ""
                ...
                if (binding.tvNoOfAddedItem.text == "Add to Cart") {
                    ...
                    sharedPreference.addSelectedProductItemModel(orderList[position]?.item_id.toString(),
                        SelectedProductItems( orderList[position]?.item_id.toString(),
                            (orderList[position]?.price ?: 0.0) as Int,1)
                    )
                    binding.llAddToCart.background = context.getDrawable(R.drawable.bg_button_rounded_green)
    ...
    inner class ProductItemViewHolder(val binding: ItemHomeProductBinding) :
